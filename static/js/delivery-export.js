...
(function () {
  ...
  function renderDrivers(summary) {
    const select = el('containerDriver');
    const previous = select.value;
    select.innerHTML = summary.drivers
      .map((d) => `<option value="${escapeHtml(d.driver_name)}">${escapeHtml(d.folder)}</option>`)
      .join('');
  ...
  function renderDayImages(summary) {
    ...
    render('loadingList', summary.day_images.loading || [], false);
  ...
  function download() {
    ...
    if (incomplete) {
      UI.toast(`${incomplete} stop(s) still missing a photo — they are listed in manifest.csv`,
               'warning', 7000);
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    el('loadingInput').addEventListener('change', async (e) => {
      ...
      if (files.length) await uploadFiles(files, 'loading', '', el('loadingStatus'));
      e.target.value = '';
    });
